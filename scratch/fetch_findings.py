...
cols, rows = query_turso(BOARD_URL, BOARD_TOKEN, "SELECT CAST(no AS INTEGER) as no_num, no, title, area, symptom, tech_what, status FROM dj_findings ORDER BY no_num ASC")

print("=== dejima dj_findings No. 1 to 60 ===")
for r in rows:
    no_num, no, title, area, symptom, tech_what, status = r
    if int(no_num) <= 60:
        print(f"[{no}] Status:{status} | Area:{area} | Title:{title}")
