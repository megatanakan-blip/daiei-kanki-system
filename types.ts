...
export interface PricingRule {
  ...
  customerId?: string;
  siteId?: string;
  customerName: string; // 移行期間のため残す
  siteName?: string;    // 移行期間のため残す
  category: string;
  model: string;
  dimensions?: string; // 表示用寸法
...
export interface Slip {
  ...
  siteId?: string;
  customerName: string; // 移行期間のため残す
  constructionName?: string; // 移行期間のため残す
  type: SlipType;
  items: SlipItem[];
  totalAmount: number;
  taxAmount: number;
  ...
  isHandled?: boolean;
  isFromReslip?: boolean;  // 欠品伝票から再作成された出庫伝票フラグ
  source?: 'link' | 'core';
  // 請求書用追加フィールド
